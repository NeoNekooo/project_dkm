...
class ImgController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $request->file('image')->storeAs(self::IMAGE_STORAGE_PATH, $filename, 'public');

            Img::create([
                'name' => $validated['name'],
                'tanggal' => $validated['tanggal'],
                'tag' => $validated['tag'],
                'path' => self::IMAGE_STORAGE_PATH . '/' . $filename
            ]);
    ...
    public function update(Request $request, Img $img)
    {
        ...
        try {
            ...
            if ($request->hasFile('image')) {
                ...
                $filename = Str::random(20) . '_' . time() . '.' . $request->file('image')->extension();
                $request->file('image')->storeAs(self::IMAGE_STORAGE_PATH, $filename, 'public');
                $updateData['path'] = self::IMAGE_STORAGE_PATH . '/' . $filename;
            }
    ...
    public function showGallery()
    {
        ...
        $imgs->setCollection(collect($formattedImgs));

        return view('pages.user.gallery', compact('imgs', 'formattedImgs', 'tags'));
    }
}
